...
- [Documentation](docs/README.md) - Subgraph development, release process, and GraphQL API reference
...
- **Versioned release deploys are automated:** merging a Release Please PR (see `.github/workflows/release-please.yml`) tags a version, deploys it to both `calibration` and `mainnet` on Goldsky, and opens a release tracking issue in the same run.
- **Production promotion is deliberate:** after both versioned subgraphs finish indexing and pass verification, follow the Subgraph Release Process to move their Goldsky `prod` tags. Downstream clients continue using the previous `prod` versions until this step.

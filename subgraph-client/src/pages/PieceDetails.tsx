...
export const PieceDetails = () => {
  ...
  const { pieceDetails, isValidPieceId, isLoading, errors } = usePiecePageData(cid, {
    retryOnError: true,
  });

  const totalDataSets = pieceDetails.length;
  const paginatedRootData = pieceDetails.slice((proofSetPage - 1) * ITEMS_PER_PAGE, proofSetPage * ITEMS_PER_PAGE);
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: intentionally re-runs only when `cid` changes
  useEffect(() => {
    setProofSetPage(1);
  }, [cid]);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto lg:px-8 lg:py-10">
      ...
      <div className="grid gap-4">
        <PieceOverviewCard pieceDetails={pieceDetails} isLoading={isLoading.details} error={errors.details} />

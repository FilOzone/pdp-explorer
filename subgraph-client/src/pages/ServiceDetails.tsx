...
export const ServiceDetails = () => {
  ...
  const totalProofSets = service?.totalProofSets ? parseInt(service.totalProofSets, 10) : 0;
  const totalProofSetsPages = Math.max(1, Math.ceil(totalProofSets / ITEMS_PER_PAGE));

  const totalProviders = service?.totalProviders ? parseInt(service.totalProviders, 10) : 0;
  const totalProvidersPages = Math.max(1, Math.ceil(totalProviders / ITEMS_PER_PAGE));
  ...
  if (serviceError) {
    return (
      <Alert variant="destructive" className="m-4">
        ...
        <AlertDescription>
          Could not load service details. Error: {serviceError.message || "Unknown error"}
        </AlertDescription>
      </Alert>
    );
  }
      ...
      <div className="mb-4">
            ...
            {serviceLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <MetricItem
                  ...
                  value={parseInt(service?.totalProofSets || "0", 10).toLocaleString()}
                />
                <MetricItem
                  title="Total Providers"
                  value={parseInt(service?.totalProviders || "0", 10).toLocaleString()}
                />
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="proofsets">
            <div className="border rounded">
              <ProofSetsTable dataSets={dataSets} isLoading={serviceLoading} error={serviceError} searchQuery="" />
            </div>

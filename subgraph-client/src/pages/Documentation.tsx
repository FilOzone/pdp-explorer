...
export const Documentation = () => {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: re-runs when `content` loads to scroll to the hash; `content` is a trigger, not read in the body
  useEffect(() => {
            ...
            <nav className="space-y-1">
              {docs.map((doc) => (
                ...
                >
                  ...
                  {doc.title}
                  {doc.id === selectedDoc.id && <ChevronRight className="h-4 w-4 ml-auto" />}
                </Link>
              ))}
            </nav>
          </div>
              ...
              <article className="prose dark:prose-invert prose-blue max-w-none">
                <ReactMarkdown
                  ...
                  components={{
                    ...
                    h1: ({ children, ...props }) => {
                      const id = createSlug(String(children).replace(/\[|\]|`/g, ""));
                      return (
                        <h1 id={id} {...props}>
                          {children}
                        </h1>

...
export const ProviderOverviewCard: React.FC<ProviderOverviewCardProps> = ({
  ...
  const totalProofsLatest = latestWeekActivities.reduce((sum, activity) => sum + Number(activity.totalProofs || 0), 0);

  const successfulProofsLatest = Math.max(0, totalProofsLatest - faultedPeriods7d);

  const lastSuccessPercent =
    totalProofsLatest > 0 ? `${((successfulProofsLatest / totalProofsLatest) * 100).toFixed(2)}%` : "0.00%";

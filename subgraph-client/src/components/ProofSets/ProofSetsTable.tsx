...
export const ProofSetsTable: React.FC<ProofSetsTableProps> = ({ dataSets, isLoading, error, searchQuery }) => {
  if (isLoading) {
  ...
  if (error) {
    return (
      <Alert variant="destructive" className="m-4">
        ...
        <AlertDescription>Could not load proof sets list. Error: {error.message || "Unknown error"}</AlertDescription>
      </Alert>
    );
  }
        ...
        <tbody>
          {dataSets.map((dataSet) => (
            <tr key={dataSet.setId} className="border-b hover:bg-gray-50 dark:hover:bg-gray-700 dark:border-gray-700">
              <td className="p-4">
                <Link to={`/dataset/${dataSet.setId}`} className="text-blue-500 hover:underline dark:text-blue-400">
                  {dataSet.setId}
                </Link>
              </td>
              <td className="p-4 font-mono text-sm">
                {dataSet.owner?.address ? (
                  ...
                  >
                    {`${dataSet.owner.address.substring(0, 10)}...${dataSet.owner.address.substring(
              ...
              <td className="p-4">
                {dataSet.lastProvenEpoch ? Number(dataSet.lastProvenEpoch).toLocaleString() : "Never"}
              </td>
              <td className="p-4">
                {dataSet.nextChallengeEpoch ? Number(dataSet.nextChallengeEpoch).toLocaleString() : "N/A"}
              </td>
            </tr>
          ))}
        </tbody>

...
import { decodeRootCid, formatDate } from "@/utility/helper";
    ...
    <TooltipProvider>
      <div className="flex flex-wrap gap-4">
        {activeRoots.map((root) => {
          ...
          return (
            <Tooltip key={root.rootId}>
              <TooltipTrigger>
                <div
                  className={`w-8 h-8 rounded cursor-pointer ${score === 0 ? "border border-gray-300" : ""}`}
        ...
        {activeRoots.length === 0 && (
          <div className="col-span-full text-center py-4 text-gray-500">No active roots found.</div>
        )}
      </div>
    </TooltipProvider>
  );

...
import { decodeRootCid, formatDataSize, formatDate } from "@/utility/helper";
  ...
  if (error) {
    return (
      <Alert variant="destructive" className="my-4">
        ...
        <AlertDescription>Could not load roots data. Error: {error.message || "Unknown error"}</AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <div className="p-4 border rounded dark:border-gray-700">
      ...
      <div className="overflow-x-auto">
        {roots.length === 0 ? (
          <div className="p-4 text-center text-gray-500 dark:text-gray-400">No roots found for this proof set.</div>
        ) : (
          <table className="min-w-full">
            ...
            <tbody>
              {roots.map((root) => {
                ...
                return (
                  <tr key={root.id} className="border-b hover:bg-gray-50 dark:hover:bg-gray-600 dark:border-gray-700">
                    ...
                    <td className="p-2">{root.lastProvenAt !== "0" ? formatDate(root.lastProvenAt) : "Never"}</td>
                    <td className="p-2">{root.lastFaultedAt !== "0" ? formatDate(root.lastFaultedAt) : "Never"}</td>
                  </tr>
                );
              })}
            </tbody>

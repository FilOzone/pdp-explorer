...
export const ServicesTable: React.FC<ServicesTableProps> = ({ services, isLoading, error, searchQuery }) => {
  ...
  if (error) {
    return (
      <Alert variant="destructive" className="m-4">
        ...
        <AlertDescription>Could not load services list. Error: {error.message || "Unknown error"}</AlertDescription>
      </Alert>
    );
  }
            ...
            <tr key={service.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
              <td className="px-6 py-4 whitespace-nowrap text-sm">
                <div className="flex items-center">
                  ...
                  >
                    <CopyableText value={service.address} truncate={true} truncateLength={6} />
                  </Link>
                  {service.address === "0x80617b65fd2eea1d7fde2b4f85977670690ed348" && (
                    <Link
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
                {parseInt(service.totalProofSets, 10).toLocaleString()}
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
                {parseInt(service.totalProviders, 10).toLocaleString()}
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
                {parseInt(service.totalRoots, 10).toLocaleString()}
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
                {formatDataSize(service.totalDataSize)}
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
                {parseInt(service.totalFaultedRoots, 10).toLocaleString()}
              </td>
            </tr>
          ))}
        </tbody>
...
const ServicesTableSkeleton: React.FC<ServicesTableSkeletonProps> = ({ itemsPerPage }) => {
  return (

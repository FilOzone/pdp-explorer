...
export const authenticate = (response: { data: string }) => {
  ...
  const expirationDate = new Date(Date.now() + 60 * 60 * 24 * 10 * 1000);
  setLocalStorage("expirationDate", expirationDate.toDateString());
};

// Format date for display
export const formatDate = (timestamp: string | number | null, showTime: boolean = false) => {
...
export function decodeWeekIdAndProviderId(concatBytes: Uint8Array) {
  ...
  const weekId = weekIdBytes[0] | (weekIdBytes[1] << 8) | (weekIdBytes[2] << 16) | (weekIdBytes[3] << 24);

...
This guide covers building the subgraph, deploying it to Goldsky, and running the `subgraph-client` app against it.

## Prerequisites

1. **Node.js and npm:** Node.js version 20.18.1 or higher. Download from [nodejs.org](https://nodejs.org/).
2. **Goldsky account:** Sign up at [goldsky.com](https://goldsky.com/). Manual deploys require a login, and production deploys require a `GOLDSKY_API_KEY` repository secret.
...
Production deploys are automated end-to-end via [release-please](https://github.com/googleapis/release-please):
...
4. In the **same workflow run** (gated on release-please's `release_created` output), a matrix `deploy` job builds and deploys to **both** `calibration` and `mainnet`, publishing to Goldsky as `pdp-explorer-calibration/<version>` and `pdp-explorer-mainnet/<version>` using the `GOLDSKY_API_KEY` repository secret.

There is no separate "promote calibration to mainnet" step. Both networks ship the same version together in one run.
